...
import '../providers/loan_category_provider.dart';
...
class _AddLoanScreenState extends ConsumerState<AddLoanScreen> {
  ...
  final _purposeController = TextEditingController(); // This will act like category
  final _durationController = TextEditingController();
  final _loanFromController = TextEditingController(); // This will act like subcategory
  ...
  void dispose() {
    ...
    _loanFromController.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    final loanCategories = ref.watch(loanCategoryProvider);
    final isDarkMode = ref.watch(themeModeProvider) == ThemeMode.dark;
    final currency = ref.watch(currencyProvider);
    final theme = Theme.of(context);

    // Loan specific theme color (Burnt Orange/Amber for Loans)
          ...
          children: [
            ...
            _BuildFormSection(
              ...
              child: _ModernDropdownField(
                ...
                hint: 'Business, Personal, etc.',
                suggestions: loanCategories.keys.toList(),
            ...
            _BuildFormSection(
              label: 'Loan From',
              child: _ModernDropdownField(
                controller: _loanFromController,
                hint: 'Bank, Friend, etc.',
                suggestions: loanCategories[_purposeController.text] ?? [],
                onChanged: (_) => setState(() {}),
                isDarkMode: isDarkMode,
              ),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: const Text('Save Loan', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ),
            ),
          ],
        ),
  ...
  void _saveLoan() {
    ...
    if (amount <= 0 || _purposeController.text.isEmpty) return;

    final loan = Loan(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      amount: amount,
      purpose: _purposeController.text,
      loanFrom: _loanFromController.text,
    ...
    // Also save the category/subcategory mapping
    if (_purposeController.text.isNotEmpty) {
      ref.read(loanCategoryProvider.notifier).addCategoryOrSubcategory(
        _purposeController.text,
        _loanFromController.text,
      );
    }
